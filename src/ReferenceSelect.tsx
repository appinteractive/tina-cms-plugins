...
import {
  ...
  useReference,
  useSelected,
} from "@/ReferenceHooks.js";
import clsx from "clsx";
import { cleanPath } from "@/utils.js";
import "./dist/reference.css";

const selectClasses =
  "flex flex-col w-full appearance-none truncate rounded-md border border-gray-300 bg-white py-2 pl-2 pr-8 text-base shadow focus:border-blue-500 focus:outline-none focus:ring-blue-500 sm:text-sm";
...
export interface wrapFieldsWithMetaProps {
  field: any;
  input: any;
  meta: any;
}

export const useReferenceSelect = ({
  field,
  input,
  meta,
}: wrapFieldsWithMetaProps) => {
  const [query, setQuery] = useState<string>("");
  const { loading, options } = useReference({
    collection: field["collection"],
    fetchMethod: (collections) => {
  ...
  const { filteredOptions, filteredStats } = useFilteredOptions(options, query);
      ...
      {({ open }) => (
        <>
          <div className="relative mt-2">
            <div
              className={clsx(selectClasses, "h-[53px] relative")}
              /* style={{
                height: "53px",
                position: "relative",
              }} */
